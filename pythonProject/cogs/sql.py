...
class LevelSystem(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        new_level = self.get_level(xp)

        if old_level == new_level:
            return

        if new_level == 2:
            role = message.guild.get_role(1035595678413508649)
            await message.author.add_roles(role)
            await message.channel.send(f"Level up! Du bist nun {role.mention}")
        ...
        async with aiosqlite.connect(self.DB) as db:
            async with db.execute(
                ...
                async for user_id, xp in cursor:
                    lvl = self.get_level(xp)
                    desc += f"{counter}. <@{user_id}> - {xp} XP - {lvl} Level\n"
                    counter += 1
